...
class TopUpRulesEngine:
    # Se inyecta el cliente en el momento de la ejecucion, no en la instanciacion
    async def evaluate(self, payload: TransactionPayload, redis_client) -> float:
        ...
        if tx_count_10m > 3: risk_penalty += 40.0
        if distinct_cards_count > 2: risk_penalty += 50.0
        if daily_total > 500.0: risk_penalty += 30.0
